...
pub async fn wasd_controller(yaml_path:&str)->Result<(), Error>
{
    let session = zenoh::open(Config::default()).res().await.unwrap();

    let bin_name = "wasd_controller";

    let pub_topic = param::get_str_param(yaml_path, bin_name, "pub_topic", "cmd_vel".to_string());
    let rate = param::get_f64_param(yaml_path, bin_name, "power_rate", 1.0) as f32;
